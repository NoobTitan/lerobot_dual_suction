...
class EliteCS66(Robot):
    ...
    def send_action(self, action: dict[str, float]) -> dict[str, float]:
        ...
        #     # 如果目标状态和实际状态不一致，发出切换命令
        #     if suction_state != float(target_suction):
        #         # 检测是否从 1 -> 0（True -> False）
        #         if hasattr(self, "_last_suction_command") and self._last_suction_command == True and target_suction == False:
        #             self.robot.interpreter("set_standard_digital_out(1, 1)")
        #             time.sleep(0.5)
        #             self.robot.interpreter("set_standard_digital_out(1, 0)")
